...
class FQE:
    """
    Fitted Q Evaluation (FQE) for policy evaluation in offline reinforcement learning.
    ...
    def avg_Q_value_est(self, state_batch: torch.Tensor, z_value: float) -> Tuple[float, float]:
        ...
        Returns:
            Tuple[float, float]: Mean Q-value and upper confidence bound
        """
...
class FQI:
    """
    Fitted Q Iteration (FQI) for constrained policy optimization in offline reinforcement learning.
...
class RLConfig_custom:
    ...
    def __init__(self, 
                 ...
                 activation_params_fqe: Dict[str, Any]) -> None:
        """
...
class RLTraining:
    ...
    def train(self, 
              ...
              save_num: int = 100,
              z_value: float = 1.96) -> Tuple[List[float], List[float], Dict[int, List[float]], 
                                               List[float], List[float], Dict[int, List[float]], 
                                               Dict[int, List[float]]]:
        """
        ...
        Returns:
            Tuple containing training metrics:
                - FQI losses
                - FQE objective losses  
                - FQE constraint losses
                - FQI estimated values
            ...
            for j in tqdm(range(self.cfg.train_eps_steps)):
                ...
                if constraint is None:
                    for m in range(len(agent_fqe_con_list)):
                        ...
                        con_est_value, con_est_value_up = agent_fqe_con_list[m].avg_Q_value_est(state_batch_val, z_value)
                        fqe_est_con[m].append(con_est_value)
                    
                    fqi_est_value = agent_fqi.avg_Q_value_est(state_batch_val)
                    avg_q_value_obj, avg_q_value_obj_up = agent_fqe_obj.avg_Q_value_est(state_batch_val, z_value)
